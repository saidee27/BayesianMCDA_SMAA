...
gg.table <- plotdata %>% spread(key = criterion, value = value) %>% gather(criterion, value, -Alternative) %>%
    ggplot(aes(x = Alternative, y = as.factor(criterion), label = value, colour = criterion)) +
        geom_text() +
        theme_bw() +
...
ggarrange(p, gg.table, ncol = 1, heights = c(4, 1))
  ...
  #                         theme(axis.text.x=element_blank(),
   #                              axis.title.x=element_blank(),
    #                             axis.ticks.x=element_blank()),
     #                    widths=c(1,8)), 
      #       arrangeGrob(arrangeGrob(nullGrob(),leg,heights=c(1,4)),tab, nullGrob(), widths=c(6,20,1)),
       #      heights=c(4,1))
...
```{r echo=FALSE}
    ...
    plotdatalist = params$res1d
    plotdata1 = plotdatalist$data2 %>% as_tibble() %>% `colnames<-`(unique(pull(params$datasetd[, params$alternatived]))) %>% mutate(Informative = c(seq(0,1,by=0.1))) %>% reshape2::melt(id.var = c('Informative'), variable.name = 'Alternative')
...
```{r fig2, fig.height=6, echo=FALSE}
 plotdata = params$res2d %>% as.matrix() %>% rowRanks() %>% as.tibble() %>% `colnames<-`(unique(pull(params$datasetd[, params$alternatived]))) %>% stack() %>% `colnames<-`(c("Rank", "Alternative")) %>% mutate(Rank = length(unique(pull(params$datasetd[, params$alternatived]))) - Rank + 1) %>% mutate(Rank = as.character(Rank)) %>% group_by(Rank, Alternative) %>% tally() %>% mutate(Rank.Probability = n/100000)
...
```{r fig3, fig.height=6, echo=FALSE}
  ...
  plotdata = params$res3d %>% as.matrix() %>% rowRanks() %>% as.tibble() %>% `colnames<-`(unique(pull(params$datasetd[, params$alternatived]))) %>% stack() %>% `colnames<-`(c("Rank", "Alternative")) %>% mutate(Rank = length(unique(pull(params$datasetd[, params$alternatived]))) - Rank + 1) %>% mutate(Rank = as.character(Rank)) %>% group_by(Rank, Alternative) %>% tally() %>% mutate(Rank.Probability = n/100000)
    p2<-ggplot(data=plotdata) + geom_bar(aes(y = Rank.Probability, x = Alternative, fill = Rank), stat="identity", width = 0.5) + ggtitle("Probability of Ranking") 
