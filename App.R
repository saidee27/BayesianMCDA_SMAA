...
ui <- {
  ...
  dashboardSidebar(
    sidebarMenu(id="tabs",
                ...
                convertMenuItem(menuItem( "Probabilistic MCDA", tabName = "pmcdas", 
                                          sliderInput("dmix", "Mixing proportion with informative prior",
                                                      min = 0, max = 1, value = 0,step = 0.05
                                          ),
                actionButton("applyp","Apply")),"pmcdas"),
                convertMenuItem(
                  menuItem( "SMAA", tabName = "smaas",
                            sliderInput("dmixS", "Mixing proportion with informative prior",
                                        min = 0, max = 1, value = 0,step = 0.05
                            ),
  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(tabName="intro",fluidRow(tabBox(title="",width='100%',height = "250px",id="methods",tabPanel("Introduction",verbatimTextOutput("introtext"),height = "400px", width = "400px"),tabPanel("MCDAimage",imageOutput("mcda1")),tabPanel("pMCDAimage",imageOutput("pmcda1")),tabPanel("SMAAimage",imageOutput("smaa1")), tabPanel("Demo",slickROutput("slickr", width = "auto")), selected=TRUE))),
...
server <-  function(input, output, session){
  ...
  dataset <- eventReactive(input$arrange, {
    ...
    if (!is.null(input$criterion_max) & !is.null(input$criterion_min)) {
      restab1 <- rawdata() %>%
        select(one_of(input$alternative, input$criterion_max, input$criterion_min, input$N, input$variance, input$Pstudy)) %>% 
        formattable(
  ...
  observeEvent(input$set, {
    ...
    lowcival = as.numeric(apply(dval - 1.96*sqrt(V),2,min,na.rm=T)) 
    highcival = as.numeric(apply(dval + 1.96*sqrt(V),2,max,na.rm=T))
 ...
 observeEvent(input$tabs, {
  ...
  res1 <- eventReactive(input$applym, {
      ...
      if(topind!=1) {
        con = mergeConstraints(lowerRatioConstraint(length(w),topind,1,100/(w[[1]][2])),upperRatioConstraint(length(w),topind,1,100/(w[[1]][1])))
        for(j in 2:length(w)) { if(topind!=j) { con = mergeConstraints(con,lowerRatioConstraint(length(w),topind,j,100/(w[[j]][2])),upperRatioConstraint(length(w),topind,j,100/(w[[j]][1])) )} }
      } else if(topind == 1) {
        con = mergeConstraints(lowerRatioConstraint(length(w),topind,2,100/(w[[2]][2])),upperRatioConstraint(length(w),topind,2,100/(w[[2]][1])))
        for(j in 3:length(w)) { con = mergeConstraints(con,lowerRatioConstraint(length(w),topind,j,100/(w[[j]][2])),upperRatioConstraint(length(w),topind,j,100/(w[[j]][1])) ) }
      }
      con = mergeConstraints(con,simplexConstraints(length(w)))
      w = hitandrun(con)
      ...
      Nvector <- dataset() %>%
        as.data.frame() %>%
         filter(.[,input$Pstudy] == 0) %>% `rownames<-`(.[, input$alternative]) %>%
        select(one_of(input$N)) %>%
        as.matrix()
      ...
      if(input$rangetyp == "customr") {
        ...
        lowcival = as.numeric(apply(dval - 1.96*sqrt(Variance),2,min,na.rm=T)) 
        highcival = as.numeric(apply(dval + 1.96*sqrt(Variance),2,max,na.rm=T))
        for (i in 1:length(allattr)) {
          if(contvars[i] == 0) lowcival[i] = max(0,lowcival[i])
          if(lowisgood[i] == 1) {
            maxvec[i] = lowcival[i]
      ...
      Ntrt = length(unique(pull(dataset()[, input$alternative])))
      Nendpts = length(c(input$criterion_max,input$criterion_min))
      
      PriorD = dataset() %>%
        as.data.frame() %>%
        filter(.[,input$Pstudy]  == 1) %>% `rownames<-`(.[, input$alternative]) %>% 
      ...
      res1 <- dataset() %>%
        ...
        select(one_of(input$criterion_max, input$criterion_min)) %>%
        as.matrix() %>%
        MCDA(Nvector=Nvector, weights = w1/sum(w1), most = maxvec,least = minvec, piece1 = piecelow, piece2 = piecemid, piece3 = piecehigh, topvar=topvar, variance = Variance, Ntrt=Ntrt, Nendpts = Nendpts, a_in = A_in, b_in = B_in, contvars = contvars, binvars = binvars)
  ...
  res2 <- eventReactive(input$applyp, {
      ...
      PriorD = dataset() %>%
        as.data.frame() %>%
        filter(.[,input$Pstudy] == 1) %>% `rownames<-`(.[, input$alternative]) %>% select(one_of(input$criterion_max, input$criterion_min)) %>%
        as.matrix()
      
      PriorN = dataset() %>%
        as.data.frame() %>%
        filter(.[,input$Pstudy] == 1) %>% `rownames<-`(.[, input$alternative]) %>% select(one_of(input$N)) %>%
        as.matrix()
      ...
      res2 <- dataset() %>%
        ...
        select(one_of(input$criterion_max, input$criterion_min)) %>%
        as.matrix() %>%
        pMCDA(Nvector=Nvector, Ntrt = Ntrt, Nendpts = Nendpts, a_in = A_in, b_in = B_in, weights = w1/sum(w1), most = maxvec,least = minvec, piece1 = piecelow, piece2 = piecemid, piece3 = piecehigh, contvars = contvars, binvars = binvars, variance = Variance, wmix = input$dmix) %>%
        as_tibble() %>%
        `colnames<-`(unique(pull(dataset()[, input$alternative])))
  })
  ...
  res3 <- eventReactive(input$applysm, {
    ...
    res3 <- dataset() %>%
      ...
      select(one_of(input$criterion_max, input$criterion_min)) %>%
      as.matrix() %>%
      SMAA(Nvector=Nvector, Ntrt = Ntrt, Nendpts = Nendpts, a_in = A_in, b_in = B_in, most = maxvec,least = minvec, piece1 = piecelow, piece2 = piecemid, piece3 = piecehigh, contvars = contvars, binvars = binvars, variance = Variance, weights = w, wmix = input$dmixS) %>%
      as_tibble() %>%
      `colnames<-`(unique(pull(dataset()[, input$alternative])))
    })
  
  output$introtext<- renderPrint({
    cat(paste0("Hi. This is a Shiny application for quantitative benefit-risk analysis using Multi Criteria Decision Analysis (MCDA), probablisitics MCDA and Stochastic Multi Criteria Acceptability Analysis (SMAA). To see a pictorial representation of these methods click on the tabs above. For a detailed demo on how to use the app and a brief explainer click on the 'Demo' tab and scroll through the slides.
...
Begin by uplodaing the data and defining relevant fields using the 'Data Source' Item on the sidebar. Once uploaded you can view the Data on the 'Arranged Data' tab.", collapse = " "))
  ...
  output$slickr <- renderSlickR({
    Dimgs<-pdftools::pdf_convert("./Demo.pdf",format = 'png',verbose = FALSE)
    slickR(Dimgs)
  })
  ...
  output$tab_plot02 <- renderPlotly({
    plotdatalist = res1()
    plotdata1 = plotdatalist$data2 %>% as_tibble() %>% `colnames<-`(unique(pull(dataset()[, input$alternative]))) %>% mutate(Informative = c(seq(0,1,by=0.1))) %>% reshape2::melt(id.var = c('Informative'), variable.name = 'Alternative')
    p02<-ggplot(data=plotdata1, aes(x=Informative, y=value, color=Alternative)) + geom_point() + geom_line() 
    p02<- p02  + labs(title = "Utility vs Weight on Informative prior in mixture", x = "Weight", y = "Net Utility")
    ggplotly(p02)
  }
  )
  ...
  output$tab_plot2 <- renderPlotly({
    plotdata = res2() %>% as.matrix() %>% rowRanks() %>% as.tibble() %>% `colnames<-`(unique(pull(dataset()[, input$alternative]))) %>% stack() %>% `colnames<-`(c("Rank", "Alternative")) %>% mutate(Rank = length(unique(pull(dataset()[, input$alternative]))) - Rank + 1) %>% mutate(Rank = as.character(Rank)) %>% group_by(Rank, Alternative) %>% tally() %>% mutate(Rank.Probability = n/100000)
  ...
  output$tab_plot5 <- renderPlotly({
    plotdata = res3() %>% as.matrix() %>% rowRanks() %>% as.tibble() %>% `colnames<-`(unique(pull(dataset()[, input$alternative]))) %>% stack() %>% `colnames<-`(c("Rank", "Alternative")) %>% mutate(Rank = length(unique(pull(dataset()[, input$alternative]))) - Rank + 1) %>% mutate(Rank = as.character(Rank)) %>% group_by(Rank, Alternative) %>% tally() %>% mutate(Rank.Probability = n/100000)
    p2<-ggplot(data=plotdata) + geom_bar(aes(y = Rank.Probability, x = Alternative, fill = Rank), stat="identity", width = 0.5) + ggtitle("Probability of Ranking") 
    ggplotly(p2)
  }
  )
